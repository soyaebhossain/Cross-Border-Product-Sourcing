  ...
  return <main className="market-shell">
    ...
    <div className="catalog-layout">
      ...
      <section className="catalog-results">
        <div className="catalog-toolbar"><div><strong>{loading ? "Loading products…" : `${data?.total || 0} ${(data?.total || 0) === 1 ? "product" : "products"}`}</strong>{query ? <span>Results for “{query}”</span> : <span>Supplier-ready global catalog</span>}</div><label className="sort-control"><span>Sort by</span><select value={sort} onChange={event => setSort(event.target.value)}><option value="recommended">Recommended</option><option value="cheapest">Lowest price</option><option value="fastest">Fastest delivery</option><option value="highest_rated">Highest rated</option><option value="name">Name A–Z</option></select></label></div>
